...
const music = new SlashCommandBuilder()
  ...
  .addSubcommand((scm) =>
    scm.setName("queue").setDescription("See the queue or make change tracks!")
  )
  .addSubcommand((scm) => scm.setName("resume").setDescription("Resume play!"))
  .addSubcommand((scm) =>
    scm
      .setName("setloop")
...
export const data = music.toJSON();
...
export const execute = async (client, interaction) => {
  ...
  if (scm === "join") {
    ...
    await play_music(client, interaction, prompts, isloop, shuffle, mode).then(
      (data) => {
        result = data;
      }
    );
